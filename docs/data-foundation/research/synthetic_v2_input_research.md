...
依据清单核验：`sources.json` 有 20 条来源，`datasets.json` 有 6 个公开数据集，`field_coverage.csv` 存在且覆盖 19 个字段；`task_evidence_map.json` 中 `stiffened-panel-fillet`、`panel-butt`、`micro-panel-web-bulkhead` 三个任务族为 `ready_for_synthetic_v2_plan`。第一版输入规范应先承接任务分类、焊缝/接头/位置/坡口/层道、工艺字段、运动字段、质量标签和证据绑定，不纳入真实参数合格性判断。
...
- 进入第一层字段：`task_taxonomy.manufacturing_stage`、`task_taxonomy.weld_object`、`task_taxonomy.joint_type`、`task_taxonomy.weld_position`、`task_taxonomy.readiness`、`task_taxonomy.access_context`。
...
- 进入第一层字段：`joint_type`、`weld_position`、`groove_geometry`、`layer_pass`、`plate_thickness_mm`、`root_gap_mm`、`leg_size_mm`、`seam_length_mm`。
...
- 进入第一层字段：`welding_process`、`current`、`voltage`、`wire_feed`、`travel_speed`、`heat_input_placeholder`、`shielding_gas`、`filler_material`、`requires_real_validation_later`。
...
- 进入第一层字段：`quality_label`、`defect_label`、`inspection_reference`、`quality_source_type`、`requires_real_validation_later`。
...
- 进入第一层字段：`source_schema.modalities`、`source_schema.quality_label_type`、`process_spec.current`、`process_spec.voltage`、`process_spec.travel_speed`、`motion_spec.motion_template`、`quality_spec.quality_label`。
...
- 进入第一层字段：`geometry_spec.groove_geometry`、`motion_spec.motion_template`、`trajectory`、`torch_angle`、`sequence_order`、`node_transition`、`process_spec.current`、`quality_spec.quality_label`。
...
- 进入第一层字段：`field_path`、`field_group`、`source_ids`、`source_category`、`simulation_role`、`evidence_role`、`value_status`、`first_batch_required`、`requires_real_validation_later`。
