...
当前判断已经从“多个候选仿真器长期平行 bake-off”调整为：**焊接工艺知识合同 + OpenUSD / Isaac Sim / Isaac Lab 是未来真实仿真训练闭环的主路线**。A02 不重复造通用物理引擎、3D 场景标准、机器人仿真器或训练框架；A02 负责焊接技能资产、工艺知识合同、证据治理、专家审查和 A01/IP handoff。

- L0 simlite 仍是稳定测试工具。
- `docs/焊接工艺数据库主要参数表.xlsx` 是 K01 焊接工艺知识合同源。
- OpenUSD 是未来数字孪生交换层。
