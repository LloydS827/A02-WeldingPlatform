...
def test_modeled_tasks_change_path_geometry_not_only_metadata():
    ...
    for task in modeled:
        ...
        assert task.variation_descriptor["policy"] == "deterministic_geometry_variation"
        assert "simulation_modeling_assumption" in task.assumption_notes
        assert "not_real_welding_process_parameter" in task.evidence_notes
```
